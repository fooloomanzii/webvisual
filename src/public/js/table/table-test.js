(function(){
  ...
  $('body').prepend('<style> .over { color: ' +
    ...
    $('#valCorner').append('<th class="sub">'+labelsArray.table.corner+'</th>');
    if(numCols<1) numCols=1;
    // Create the array of labels for the values
    for(var i = 0; i<numCols; i++){
      $('#valCols').append('<th class="subheader">' +
          (labelsArray.data.subtypes[i]||i+1) + '</th>');
    }
  }
  ...
  function arrangeData(data, exceeds){
    ...
    for(var i=0; i<parseInt(valuesArray.length/numCols, 10); ++i) {
      ...
      '<th class="valGroup subheader">'+
      (labelsArray.data.types[i]||labelsArray.table.unnamedRow+' '+(i+1))+'</th>';
      for(var j=numCols*i; j<numCols*(i+1);j++){
        tmp+='<td id="value'+j+'" class="'+checkColor(j)+'">'+
        valuesArray[j]+'</td>';
      }
      tmp+='</tr>';
